...
public class FaceManager {
    ...
    public Face[] getFaces(String filePath) {
        FaceInfo info = getFaceInfo(filePath);
        if (info != null && info.mHasFace) {
            Log.d(TAG, "<getFaces>filePath = " + filePath+" Number = " + info.mFaceNumber);
            Face[] faces = new Face[info.mFaceNumber];
            for (int i = 0; i < info.mFaceNumber; i++) {
                String faceName = info.faceLists.get(i).faceName;
                String faceID = info.faceLists.get(i).faceID;
                RectF faceRectF = info.faceLists.get(i).mRect;
                Rect faceRect = new Rect((int) faceRectF.left, (int) faceRectF.top, (int)
                        faceRectF.bottom, (int) faceRectF.right);
                faces[i] = new Face(faceName, faceID, faceRect);
                Log.d(TAG, "<getFaces>faceName = " + faceName + " faceID = " + faceID + "Rect = " +
                        faceRect.toShortString());
            }
            return faces;
        }
        return null;
    ...
    private void createTexture(FaceInfo faceInfo, Bitmap background) {
        if (faceInfo != null && faceInfo.mHasFace) {
            Bitmap faceBitmap = Bitmap.createBitmap(faceInfo.mBitmapWidth, faceInfo
    ...
    private void createFaceThumbNail(FaceInfo faceInfo, Bitmap background) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            info.getFaceThumbNailPath = Utils.DUMP_PATH + "" + info.faceThumbNail + "." + Utils
                    .SUFFIX;
